...
namespace Gilbertsoft\HostingUtility\Utility;
...
class DnsUtility
{
    ...
    /**
     * @param string $zone
     * @return array MX hosts
     */
    public static function getMxHosts(string $zone): array
    {
        $mxRecords = dns_get_record($zone, DNS_MX);
        $mxHosts = [];

        foreach ($mxRecords as $key => $value) {
            $mxHosts[$value['pri']] = $value['target'];
        }
    ...
    /**
     * @param string $zone
     * @return string Primary MX host
     */
    public static function getPrimaryMXHost(string $zone): string
    {
        $mxHosts = self::getMxHosts($zone);
     ...
     * @return array
     */
    public static function getHosts(int $type, string $hostname): array
    {
        return dns_get_record($hostname, $type);
    }

    private static function filterRecords(array $ignoreKeys, array $records): array
    {
    ...
    private static function sortRecordsCmp($a, $b): int
    {
    ...
    public static function checkRecords(
        ...
        $records = static::getHosts($type, $hostname);
